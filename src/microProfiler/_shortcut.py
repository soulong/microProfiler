"""Create a Windows Start Menu shortcut for the microProfiler GUI application.

The shortcut launches microProfiler inside the ``micro`` conda environment.
"""
...
def _find_icon_ico() -> Path | None:
    ...
    try:
        ...
        return Path(str(files("microProfiler.resources") / "icon.ico"))
    except Exception:
        return None
...
def create_shortcut() -> None:
    """Create shortcut that launches microProfiler in the *micro* conda env.

    Uses ``pythonw.exe`` (windowless Python) to avoid a visible console
    window.  Falls back to ``conda run`` if the environment cannot be
    located directly.

    Called via the ``microprofiler-install-shortcut`` console command.
    """
    ...
    if pythonw:
        ...
        if conda_exe is None:
            ...
            return
        target = conda_exe
        args = "run -n micro --no-capture-output microprofiler"

    work_dir = str(Path.home())

    link_dirs = [_startup_menu(), _desktop()]
    for link_path in [d / "microProfiler.lnk" for d in link_dirs]:
